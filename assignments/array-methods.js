...
firstNamesAllCaps.push(runners.map(object => object.first_name.toUpperCase()));
...
runnersLargeSizeShirt.push(runners.filter(object => object.shirt_size === 'L'));

console.log(runnersLargeSizeShirt);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations and save the total into a ticketPriceTotal variable.
let ticketPriceTotal = runners.reduce(function(total, eachRunner) {
  return total += eachRunner.donation;
}, 0);
...
runners.forEach(function (object) {
  let runner = `${object.first_name} ${object.last_name} is runner # ${object.id}`;
  nameAndNum.push(runner)
});
